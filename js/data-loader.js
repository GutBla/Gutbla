const DataLoader = (() => {
    ...
    async function fetchJSON(relativePath) {
        ...
        if (!res.ok) throw new Error(`No se pudo cargar ${relativePath} (${res.status})`);
        return res.json();
    }
    ...
    function buildProjectItem(project) {
        ...
        el.dataset.image       = project.image;
    ...
    async function loadProyectos() {
        ...
        grid.innerHTML = '<p style="color:var(--text-gray);padding:20px">Cargando proyectos…</p>';
        ...
        try {
            ...
            grid.innerHTML = '<p style="color:var(--text-gray);padding:20px">No se pudieron cargar los proyectos.</p>';
        }
    }
    ...
    async function loadRecursos() {
        ...
        container.innerHTML = '<p style="color:var(--text-gray);padding:20px">Cargando recursos…</p>';
        ...
        try {
            ...
            data.tabs.forEach((tab, i) => {
                ...
                tab.sections.forEach(section => {
                    ...
                    section.items.forEach(item => innerGrid.appendChild(buildResourceItem(item)));
            ...
            container.innerHTML = '<p style="color:var(--text-gray);padding:20px">No se pudieron cargar los recursos.</p>';
        }
    }
...
document.addEventListener('DOMContentLoaded', () => {
    if (document.querySelector('.gallery-grid.projects-json-target')) DataLoader.loadProyectos();
    if (document.querySelector('.resources-json-target'))             DataLoader.loadRecursos();
});